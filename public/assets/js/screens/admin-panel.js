...
export async function renderAdminPanel(container) {
  ...
  container.innerHTML = `
    ...
    <div class="page screen-enter">
      <div class="tabs" id="admin-tabs">
        ...
        <button class="tab-btn" data-tab="versus">COMPARATIVO</button>
        <button class="tab-btn" data-tab="compare">UNIDADES</button>
  ...
  const renderers = {
    ...
    versus:    () => renderVersus(tabContent),
...
// ─── Tab: Comparativo de Estoquistas ─────────────────────────────────────────
//
// Quatro gráficos de barras VERTICAIS comparando cada estoquista em:
//   • PEDIDOS          — pedidos processados (lotes + avulsos)
//   • ITENS SEPARADOS  — peças retiradas na fase de separação (BATCH + ONLY_SEPARATION + SINGLE_ORDER)
//   • CAIXAS LACRADAS  — caixas fechadas na bipagem (boxCodes de BATCH + ONLY_BIPPING + SINGLE_ORDER)
//   • ITENS BIPADOS    — peças conferidas/lacradas na bipagem (BATCH + ONLY_BIPPING + SINGLE_ORDER)
//
// IMPORTANTE: "itens" = SOMA DAS QUANTIDADES (peças), não nº de materiais distintos.
// Ex.: 1 material com quantidade 5 conta como 5 itens. Mesma convenção usada no
// parser (spreadsheet-parser.js) e nas telas de gravação (somam o.items por pedido).
async function renderVersus(el) {
  const spinner = `<div class="text-center mt-4"><div class="spinner" style="margin:0 auto;"></div></div>`;
  ...
  // Mapa de nomes: unitId → nome da unidade; unitId_stockistId → nome do estoquista.
  const nameMap = {};
  units.forEach(u => {
    nameMap[u.id] = u.name;
    (u.stockists || []).forEach(s => { nameMap[u.id + '_' + s.id] = s.name; });
  });

  const INDICATORS = [
    { key: 'pedidos',        label: 'PEDIDOS',         icon: '📋', color: '#00d4ff', hint: 'Pedidos processados (lotes + avulsos).' },
    { key: 'itensSeparados', label: 'ITENS SEPARADOS', icon: '📦', color: '#00ff88', hint: 'Peças retiradas na separação.' },
    { key: 'caixas',         label: 'CAIXAS LACRADAS', icon: '🔒', color: '#d97706', hint: 'Caixas fechadas na bipagem.' },
    { key: 'itens',          label: 'ITENS BIPADOS',   icon: '🔢', color: '#c084fc', hint: 'Peças conferidas/lacradas na bipagem.' },
  ];
  ...
  // Agrega métricas por estoquista. Chave = unitId_stockistId (evita colisão de
  // ids iguais entre unidades). Segue a mesma lógica de computeRanking/computeKpis.
  function aggregate(events) {
    const map = {};
    function ensure(unitId, sid) {
      const key = unitId + '_' + sid;
      if (!map[key]) {
        map[key] = {
          key,
          name: nameMap[unitId + '_' + sid] || sid,
          unitName: nameMap[unitId] || unitId,
    ...
    for (const ev of events) {
      if (!ev.stockistId) continue;
      const rec = ensure(ev.unitId, ev.stockistId);
      rec.xp += ev.xp || 0;
  ...
  function buildBarChart(canvas, { labels, fullNames, unitNames, values, color, showUnit }) {
    if (typeof Chart === 'undefined' || !canvas) return null;
    try { Chart.getChart(canvas)?.destroy(); } catch { /* ignore */ }
    ...
    return new Chart(ctx, {
      ...
      options: {
          ...
          tooltip: {
            callbacks: {
              title: (items) => {
                ...
                return showUnit ? `${fullNames[i]} · ${unitNames[i]}` : fullNames[i];
              },
            },
          },
        },
        scales: {
          x: {
            ticks: { color: '#9ca3af', font: { family: 'Share Tech Mono', size: 10 }, maxRotation: 60, minRotation: 0 },
            grid: { display: false },
          },
          y: {
            beginAtZero: true,
            ticks: { color: '#6b7280', font: { family: 'Share Tech Mono' }, precision: 0 },
            grid: { color: '#2a2a3a' },
          },
        },
      },
  ...
  function render(records) {
    const showUnit = unitFilter === 'all' && units.length > 1;

    el.innerHTML = `
      <div class="card cyber-chamfer mb-2" style="padding:1rem;">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.5rem;">
          <div class="section-title" style="margin:0;">⚔ COMPARATIVO DE ESTOQUISTAS</div>
          <div style="display:flex;gap:0.4rem;flex-wrap:wrap;align-items:center;">
            <select id="versus-unit" class="input input--no-prefix" style="width:150px;padding-left:0.6rem;font-size:0.7rem;">
              <option value="all" ${unitFilter === 'all' ? 'selected' : ''}>Todas unidades</option>
              ${units.map(u => `<option value="${u.id}" ${unitFilter === u.id ? 'selected' : ''}>${u.name}</option>`).join('')}
            </select>
            ${periodButtons(period)}
          </div>
        </div>
        <div class="text-muted text-xs mt-2" style="font-family:var(--font-terminal);letter-spacing:0.05em;line-height:1.5;">
          Barras verticais comparando cada estoquista. <strong>Itens = soma das quantidades (peças)</strong>,
          não o número de materiais distintos — ex.: 1 material com qtd. 5 conta como <strong>5 itens</strong>.
          ${records.length} estoquista(s) com atividade no período.
        </div>
      </div>
      ...
      ${records.length === 0
        ? `<div class="card cyber-chamfer text-center" style="padding:2rem;color:var(--muted-fg);font-family:var(--font-terminal);font-size:0.8rem;">Sem atividade de estoquistas no período.</div>`
        : `
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));gap:1rem;">
          ${INDICATORS.map(ind => `
            <div class="card cyber-chamfer">
              <div class="section-title mb-1" style="display:flex;align-items:center;gap:0.5rem;">
                <span>${ind.icon}</span><span>${ind.label}</span>
              </div>
              <div class="text-muted text-xs mb-2" style="font-family:var(--font-terminal);">${ind.hint}</div>
              <div style="position:relative;height:260px;">
                <canvas id="chart-${ind.key}"></canvas>
              </div>
            </div>
          `).join('')}
        </div>
    ...
    el.querySelector('#versus-unit')?.addEventListener('change', (e) => {
    ...
    if (typeof Chart === 'undefined') {
      el.querySelectorAll('canvas').forEach(c => {
        c.parentElement.innerHTML = '<div class="text-muted text-xs text-center" style="padding:2rem;">Gráfico indisponível (Chart.js não carregou).</div>';
      });
      return;
    }

    const shortNames = records.map(r => r.name.split(' ')[0]);
    const fullNames  = records.map(r => r.name);
    const unitNames  = records.map(r => r.unitName);

    INDICATORS.forEach(ind => {
      const canvas = el.querySelector(`#chart-${ind.key}`);
      const chart = buildBarChart(canvas, {
        labels: shortNames, fullNames, unitNames,
        values: records.map(r => r[ind.key]),
        color: ind.color, showUnit,
      });
  ...
  async function load() {
    ...
    let events;
    try {
      events = await getAllEvents({ startDate, endDate, maxDocs: 1000 });
    } catch (err) {
    ...
    if (unitFilter !== 'all') events = events.filter(e => e.unitId === unitFilter);
